  ...
  if (!_.isNull(message)) {
    ...
    if (_.isNull(message.prefix)) {
      ...
      switch (message.command) {
        ...
        case 'NOTICE':
          switch (msgid) {
            ...
            case 'delete_message_success':
              this.log.info('[' + channel + '] ' + msg);
              this.emits(['notice', '_promiseDelete'], [[channel, msgid, msg], [null]]);
              break;
            ...
            case 'bad_delete_message_mod':
              this.log.info('[' + channel + '] ' + msg);
              this.emits(['notice', '_promiseDelete'], [[channel, msgid, msg], [msgid]]);
              break;
        ...
        case 'USERNOTICE':
          {
            if (msgid === 'resub') {
              ...
              this.emit('subgift', channel, msgid === 'anonsubgift' ? null : _username2, recipient, { plan: _plan2, planName: _planName2 }, _userstate2);
            } else if (msgid === 'submysterygift' || msgid === 'anonsubmysterygift') {
              var _username2a = message.tags['display-name'] || message.tags.login;
              var _userstate2a = message.tags;

              if (_userstate2a) {
                _userstate2a['message-type'] = 'submysterygift';
              }

              this.emit('submysterygift', channel, msgid === 'anonsubmysterygift' ? null : _username2a,  _userstate2a);
            } else if (msgid === 'giftpaidupgrade' || msgid === 'anongiftpaidupgrade') {
              var _username2a = message.tags['display-name'] || message.tags.login;
              var _userstate2a = message.tags;

              if (_userstate2a) {
                _userstate2a['message-type'] = 'giftpaidupgrade';
              }

              this.emit('giftpaidupgrade', channel, msgid === 'anongiftpaidupgrade' ? null : _username2a,  _userstate2a);
            } else if (msgid === 'raid') {
        ...
        case 'CLEARMSG':
          var login = _.get(message.tags['login'], null);
          var targetMessageId = _.get(message.tags['target-msg-id']);

          this.log.info('[' + channel + '] Message from ' + login + ' removed. ID: ' + targetMessageId + ' Message: ' + msg);
        ...
        case 'PRIVMSG':
          ...
          if (message.tags.username === 'jtv') {
            ...
            if (msg.match(/^\u0001ACTION ([^\u0001]+)\u0001$/)) {
              ...
              this.emits(['chat', 'message', 'raw'], [[channel, message.tags, msg, false], [channel, message.tags, msg, false], [message]);
            }
          }
          break;
