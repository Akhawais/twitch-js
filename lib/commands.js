...
module.exports = {
  ...
  deletemessage: function deletemessage(channel, id) {
    ...
    // Send the command to the server and race the Promise against a delay..
    return this._sendCommand(this._getPromiseDelay(), channel, '/delete ' + id, function (resolve, reject) {
      // Received _promiseDelete event, resolve or reject..
      _this21.once('_promiseDelete', function (err) {
        if (!err) {
          resolve([channel, id]);
        } else {
          reject(err);
  ...
  whisper: function whisper(username, message) {
    ...
    return this._sendCommand(this._getPromiseDelay(), '#tmijs', '/w ' + username + ' ' + message, function (resolve) {
      ...
      var userstate = _.merge({
        ...
        'message-id': null,
        'thread-id': null,
        username: _this26.getUsername()
      }, _this26.globaluserstate);

      // Emit for both, whisper and message..
      _this26.emits(['whisper', 'message'], [[from, userstate, message, true], [from, userstate, message, true]]);
